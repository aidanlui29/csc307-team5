...
const allowedOrigins = [
  "http://localhost:5173"
  // later add your deployed frontend URL here, e.g. "https://<something>.azurestaticapps.net"
];
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const docs = await Event.find(query).sort({
        date: 1,
        startMin: 1
      });
  ...
  async (req, res) => {
    try {
      ...
      if (e <= s)
        return res
          .status(400)
          .send("End time must be after start time");
  ...
  async (req, res) => {
    try {
      ...
      if (req.body?.desc !== undefined)
        update.desc = String(req.body.desc);

      if (req.body?.startMin !== undefined) {
        const s = toMinuteInt(req.body.startMin);
        if (s === null)
          return res.status(400).send("Invalid startMin");
        update.startMin = s;
      }
...
(async function connectDbInBackground() {
  try {
    if (!process.env.MONGODB_URI) {
      throw new Error(
        "MONGODB_URI is missing (set it in Azure App settings)."
      );
    }
    await connectDb();
