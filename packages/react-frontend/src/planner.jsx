...
function monthTitleForWeek(weekStart) {
  ...
  if (weekStart.getMonth() === weekEnd.getMonth() && startYear === endYear) return `${startMonth} ${startYear}`;
      ...
      >
        <div className="planner__gridwrap" ref={gridRef}>
          ...
          {(loadingEvents || pageError) && (
            <div style={{ padding: 12, opacity: 0.9 }}>{loadingEvents ? "Loading events..." : pageError}</div>
          )}
          ...
          {hours.map((hour) => (
            <div key={hour} className="planner__row">
              ...
              {weekDates.map((_, idx) => (
                <div key={`${idx}-${hour}`} className={`planner__cell ${isCurrentWeek && idx === todayIndex ? "planner__cell--today" : ""}`} />
              ))}
            </div>
          ))}
        </div>
      ...
      {addOpen && (
        <div className="plannerModal" role="dialog" aria-modal="true" onClick={closeAdd}>
          ...
          <div className="plannerModal__card" onClick={(e) => e.stopPropagation()}>
            ...
            <input className="plannerModal__title" placeholder="title" value={title} onChange={(e) => setTitle(e.target.value)} />
